...
def getTopList(dsName):
    top5000List = []
    #topPath = 'D:/Alfonso Ngan/Documents/Github Project/Sketch-for-Data-Stream/data/top5000_'
...
for ds in dataset:
    ...
    topList = []; radList = []
    top5000List.sort(key= lambda d : d[2], reverse = True) # from large to small
    ...
    tem = {'h':h,'w':w,'ds':ds[2],'sketch':'hSketch'}
    print('----------hSketch')
    ...
    print('start rad')
    tem1 = {'h':h,'w':w,'ds':ds[2],'sketch':'hSketch'}
    print('----------hSketch')
    for j in range(len(radList)):
        tem1[radNum[j]] = {}
        rad_mean = []
        rad_medium = []
        rad_sum = []
